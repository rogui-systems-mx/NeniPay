...
const getOptimizedBase64 = async (uri: string): Promise<string | null> => {
    ...
    try {
        ...
        if (uri.startsWith('http')) {
            ...
            isTempFile = true;
        } else if (Platform.OS !== 'web' && !uri.startsWith('file://') && !uri.startsWith('content://') && !uri.startsWith('data:')) {
            // Add file:// prefix if missing for local paths (Native only)
            localUri = `file://${uri}`;
        }
...
export const generateCatalogPDF = async (products: Product[], businessName: string) => {
    // Generate filename with date: catalogo_DD_MM_YYYY.pdf
    const now = new Date();
    ...
    // Filter out products without stock if desired
    ...
    try {
        if (Platform.OS === 'web') {
            ...
            // Rename the file to the desired catalogo_DD_MM_YYYY.pdf format
